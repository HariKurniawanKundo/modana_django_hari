...
class UsersCreateAPITest(APITestCase):
    ...
    def test_create_first_user(self):
        response = self.factory.post(
            "/api/users/", data=json.dumps(self.first_payload), format="json",
        )

    def test_create_second_user(self):
        response = self.factory.post(
            "/api/users/", data=json.dumps(self.second_payload), format="json",
        )

    def test_create_third_user(self):
        response = self.factory.post(
            "/api/users/", data=json.dumps(self.third_payload), format="json",
        )

    def test_create_fourth_user(self):
        response = self.factory.post(
            "/api/users/", data=json.dumps(self.fourth_payload), format="json",
        )

    def test_create_fifth_user(self):
        response = self.factory.post(
            "/api/users/", data=json.dumps(self.fifth_payload), format="json",
        )

    def test_create_sixth_user(self):
        response = self.factory.post(
            "/api/users/", data=json.dumps(self.sixth_payload), format="json",
        )

    def test_create_seventh_user(self):
        response = self.factory.post(
            "/api/users/", data=json.dumps(self.seventh_payload), format="json",
        )

    def test_create_eight_user(self):
        response = self.factory.post(
            "/api/users/", data=json.dumps(self.eight_payload), format="json",
        )

    def test_create_last_user(self):
        response = self.factory.post(
            "/api/users/", data=json.dumps(self.last_payload), format="json",
        )
